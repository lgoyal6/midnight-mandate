...
> A contract-held payment executes only when the exact amount and recipient satisfy a private committed mandate.
...
- `policy_commitment` — opaque commitment to secret, cap, and allowed recipient;
...
[`src/witnesses.ts`](../src/witnesses.ts) supplies `policy_secret`, `max_per_payment`, and `allowed_recipient` from the local private-state provider during proof generation. A secure circuit constrains each witness; it never treats witness data as trusted merely because it is private.
...
2. A compiled pure circuit derives the commitment from secret, cap, and canonical recipient bytes.
...
5. It checks allowed recipient, positive amount, private cap, unused nonce, and vault balance.
6. It inserts the nullifier/receipt and debits the mirror.
...
Over-cap, wrong-recipient, wrong-opening, zero, insufficient-balance, and replay failures occur before a successful transaction. Tests snapshot vault balance, payment count, nullifier/receipt sizes, and recipient balance; every required rejection must leave them unchanged.
