...
| Delegated proving runtime | Witness values: secret, cap, and allowed recipient |
...
| Public ledger/indexer | Commitment, nullifiers, receipts, counters, custody balance, and successful unshielded settlement |
...
A response scan rejected the keys `maxPerPayment`, `allowedRecipient`, `policySecret`, `instruction`, `proposal`, and `attack`. Browser verification also confirmed that the observer DOM contains neither the private cap display, preset address, nor invoice text.
